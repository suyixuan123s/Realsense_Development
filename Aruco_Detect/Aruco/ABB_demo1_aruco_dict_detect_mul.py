...
def fitPlane(points):
    ...
    :param points: 一个 Nx3 的 NumPy 数组，表示 N 个 3D 点
    :return  plane_normal: 平面的法向量
             plane_d: 平面方程的常数项
    """
...
def visualize_plane_in_image(image, points_3d, camera_matrix, dist_coeffs, color=(0, 255, 0), thickness=2):
    ...
    :param image: 要绘制的二维图像 (NumPy 数组)
    :param points_3d: 四个三维点的 NumPy 数组，形状为 (4, 3)。  这些点定义了平面区域的四个角点
    :param camera_matrix: 相机内参矩阵
    :param dist_coeffs: 相机畸变系数
    :param color: 绘制线条的颜色 (BGR 格式)
...
def plane_to_camera_transform(plane_normal, points_3d):
    ...
    :param plane_normal: 平面法向量 (NumPy 数组，形状为 (3,))
    :param points_3d: 四个二维码中心点的三维坐标 (NumPy 数组，形状为 (4, 3))
    :return  R_plane_to_camera: 旋转矩阵 (NumPy 数组，形状为 (3, 3))
...
marker_size = 0.025
# 读取图像
image = cv2.imread(r'../../Task1_Intel_Realsense_D435/Data_Intel_Realsense_D405/color_image_20250410-172926.jpg')
# 转换为灰度图像
...
gray_undistorted = cv2.undistort(gray, camera_matrix, dist_coeffs, None, newcameramtx)

# 检测标记
corners, ids, _ = aruco.detectMarkers(gray_undistorted, aruco_dict, parameters=parameters)
print(corners)
...
if ids is not None:
    ...
    print("Center在相机坐标系下的三维坐标:", box_center)
    ...
    print("center在图像上的二维坐标:", center_2d)
    # 将投影的中心点转换为整数坐标
    center_2d = center_2d[0][0].astype(int)
    print("center在图像上的二维整数值坐标", center_2d)
    print()
    # 在图像上绘制中心点
    cv2.circle(image, (center_2d[0], center_2d[1]), radius=10, color=(0, 0, 255), thickness=-1)  # 红色圆点

    pixel_position_box_center = camera_to_pixel_coordinates(box_center, K)
    print("图像坐标系中的像素位置:", pixel_position_box_center)
    ...
    #
    # average_depth_x = (tvecs[x_min_index, 2] + tvecs[x_max_index, 2]) / 2
    # average_depth_y = (tvecs[y_min_index, 2] + tvecs[y_max_index, 2]) / 2
    #
    # real_width = (pixel_width / camera_matrix[0, 0]) * average_depth_x  # 使用 x_min 的深度
    # real_height = (pixel_height / camera_matrix[1, 1]) * average_depth_y  # 使用 y_min 的深度
    #
    # print("包围框的宽度 pixel_width:", pixel_width)
    # print("包围框的高度 pixel_height:", pixel_height)
    # print("实际包围框的宽度real_width:", real_width)
    # print("实际包围框的高度 real_height:", real_height)
    #
    # # 打印对应的二维码 ID
    ...
    # 计算plane_to_camera_transform 转换矩阵
    ...
    # ----------------  计算像素坐标到相机坐标的转换 --------
    # 物体在图像中的像素坐标
    # u, v = 280, 210  # 替换为实际的像素坐标
    u, v = 280.32301343, 210.46300673
    pixel_point = (u, v)
    ...
    # # print("y_norm", y_norm)
    #
    # x_norm = -0.09647282400050729
    # y_norm = -0.06088540417074822

    # ------------- 深度值的计算 -----------
    depth_map = cv2.imread('../../Task1_Intel_Realsense_D435/Data_Intel_Realsense_D405/depth_image_20250410-172926.png',
                           cv2.IMREAD_UNCHANGED)  # 读取深度图
    # 获取该像素的深度值
    # depth_value = depth_map[v, u] * depth_scale
    depth_value = 0.20624314
    print(f"在像素坐标系下(u, v)({u}, {v})的深度值为: {depth_value}\n")
    camera_coords = pixel_to_camera_coordinates(pixel_point, K, depth_value)
    ...
    for i in range(len(ids)):
        ...
        P_object_plane = np.array([0, 0, 0])
        Transformation_Matrix = np.array(transformation_matrix)
        P_object_plane_homogeneous = np.append(P_object_plane, 1)
        P_object_camera = Transformation_Matrix @ P_object_plane_homogeneous
        P_object_camera2_3D = P_object_camera[:3]
        np.set_printoptions(suppress=True)
        print(f"{ids[i][0]}:", P_object_camera2_3D)

        pixel_position = camera_to_pixel_coordinates(P_object_camera2_3D, K)
        print("图像坐标系中的像素位置:", pixel_position)

        pixel_position = transform_point_to_image(P_object_plane, Transformation_Matrix, K)
        print(f"物体坐标系下的点{ids[i][0]}:像素位置:", pixel_position)
    ...
    average_transformation_matrix = np.eye(4, dtype=np.float64)
    ...
    P_object_plane_homogeneous1 = np.append(P_object_plane1, 1)
    P_object_camera1 = Transformation_Matrix @ P_object_plane_homogeneous1
    P_object_camera2_3D1 = P_object_camera1[:3]
    np.set_printoptions(suppress=True)
    print("平面上的点转换为相机坐标系下的坐标为", P_object_camera2_3D1)

    pixel_position = transform_point_to_image(P_object_plane1, average_transformation_matrix, K)
